  ...
  if (data) {
    ...
    return (
      <>
        ...
        <div className="pannel h-24 w-full left-0 top-0 flex overflow-hidden justify-evenly bg-[#bddad295] max-sm:justify-start">
          <div className="flex w-full sm:justify-evenly max-sm:animate-marquee ">
            {allData?.map((item: any, index: number) => (
              <div
                key={index}
                className="h-20 min-w-20 m-2 rounded-md cursor-pointer shrink-0"
                style={{
                  backgroundImage: `url(${BASE_URL}${API_URL.TEAM.BANNER}/${encodeURIComponent(item.banner)})`,
        ...
        <div
          className="maincont h-72 w-full hidden sm:flex justify-evenly "
          style={{
        ...
        >
          <div className="left sm: h-72 w-1/2 my-auto flex ">
            <img
        ...
        >
          <div className="left sm: h-25 w-1/2 my-auto  ">
            <img
              src={`${BASE_URL}${API_URL.TEAM.BANNER}/${encodeURIComponent(
                res.logo,
              )}`}
              alt="logo"
              className="h-20 w-20 my-auto ml-16 coin"
              style={{ transformStyle: "preserve-3d" }}
            />

            <div className="right my-auto">
              <h1 className="text-2xl font-bold text-center">{res.name}</h1>
              <p className="flex mt-2 h-16 w-auto flex-wrap justify-center">
                {trophy.map((item: any, index: number) => (
                  <img
                    key={index}
                    src="https://www.iplt20.com/assets/images/teams-trophy.svg"
